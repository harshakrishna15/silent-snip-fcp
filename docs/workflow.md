...
Cutdown is a local, audio-only silence-removal tool for Final Cut Pro. The user adds an audio effect, analyzes one selected clip, reviews proposed cuts, and explicitly applies them by replacing the current project through XML. The effect itself passes audio through unchanged; the helper performs analysis and project replacement.
...
4. Sends exactly one edited project with the original project name to its original event and library. It accepts Final Cut’s matching-library **Replace** prompt only for this fresh delivery, after checking the original project and a unique browser item. It does not choose **Keep Both**.
5. Reopens the replacement (Final Cut may close the old timeline), re-exports it, and verifies the original event/library, a new host project identity, timeline, media references, effects, and controller settings before reporting completion.

Import delivery is asynchronous. The helper waits for the identified replacement confirmation and for the result to become available; seeing the original same-named project is not completion. It records dialogs in `Import-Dialogs.json`. Only the matching replacement decision and the identified completed-import warning are handled automatically. Unrecognized dialogs remain open. Retry Verification never imports again or accepts a replacement prompt.

The source media remains unchanged. The project is replaced under its existing name in its existing event, so repeated Apply runs should not add result projects. Final Cut assigns a new internal project identity; this is replacement, not an in-place edit preserving that identity or a promised single Undo operation. The pre-Apply project is saved as `Before-Cuts.fcpxml` on disk, not automatically imported as another project. Retained audio stays editable and references the existing media.

**September 22 validation:** manual host XML replacement probes in this checkout’s required integration library kept one project across repeated replacements. This establishes the host’s name-based replacement behavior, not an automatic Analyze/Apply run of the updated helper. See [replacement evidence and limitations](xml-replacement.md).
...
Listen around cut boundaries and continue editing in the replaced project. To recover the pre-Apply edit, import that run’s `Before-Cuts.fcpxml`; it has a distinct recovery name and identity. Recovery history stays in the result folders until explicitly imported.
...
If Apply stops before delivery, no result import was requested. If delivery was attempted but verification fails or is interrupted, the original may already have been replaced: check the current project before importing again. Lost Apply messages are retried using the same job identity and cannot apply a consumed plan twice. After 15 seconds without an accepted response, the window disables the old review and permits fresh Analyze; first check the current project because replacement may already have happened. A running helper operation still blocks a new analysis until it finishes. Existing result folders are not overwritten.
